...
# Función para mostrar la gráfica de los datos
def mostrar_grafica():
    if not datos_modelo:
    ...
    # Subgráfico para la velocidad de la bala
    plt.subplot(3, 1, 1)
    plt.plot(velocidades, label='Velocidad de la Bala')
    plt.ylabel('Velocidad')
    plt.legend()

    # Subgráfico para la distancia entre la bala y el jugador
    plt.subplot(3, 1, 2)
    plt.plot(distancias, label='Distancia al Jugador', color='orange')
    plt.ylabel('Distancia')
    plt.legend()

    # Subgráfico para los saltos realizados
    plt.subplot(3, 1, 3)
    plt.plot(saltos, label='Salto (1=Sí, 0=No)', color='green')
    plt.xlabel('Tiempo')
    plt.ylabel('Salto')
    plt.legend()
...
# Función para mostrar el menú y seleccionar el modo de juego o graficar los datos
def mostrar_menu():
        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_a:
                    ...
                    modo_auto = False
                    menu_activo = False
                elif evento.key == pygame.K_g:  # Nueva opción para graficar los datos
                    mostrar_grafica()
