...
function datosDeEntrenamiento(param_entrada) {
    ...
    console.log("Valor", "En la salida %:  %: " + aire);
...
function colisionH() {
    // Llamar a la función para generar el CSV automáticamente al perder
    guardarDatosCSV();
    pausa();
}
...
function guardarDatosCSV() {
    // Encabezados del CSV
    let csvContent = "data:text/csv;charset=utf-8,despBala,velocidadBala,estatusAire,estatuSuelo\n";
    
    // Recorrer el arreglo de datos de entrenamiento y agregarlo al contenido CSV
    datosEntrenamiento.forEach(function(row) {
        let fila = row.input.concat(row.output).join(","); // Unimos la entrada y salida en una sola línea separada por comas
        csvContent += fila + "\n"; // Agregamos un salto de línea al final de cada fila
    });

    // Crear un archivo Blob y generar una URL para descargarlo
    var encodedUri = encodeURI(csvContent);
    var link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "datosEntrenamiento.csv");
